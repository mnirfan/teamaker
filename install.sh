...
user=`whoami`
if [ $user == "root" ]; then
	echo -e "\033[33m All files in destination will be overwriten, continue?\e[00m \n1. Yes\nelse. No\n"
	read -r confirm
	if [ $confirm == '1' ]; then
	
		gcc `pkg-config --cflags gtk+-3.0` ./Files/main.c -o ./Files/teamaker `pkg-config --libs gtk+-3.0`
		
		if [ -f "Files/teamaker" ]; then
			rm -r /usr/share/TEA
			...
			rm Files/teamaker
			echo -e "Tea package maker has been successfully installed\n\n"
		else
			echo -e "Compilation error, check your compiler\n"
		fi
	else
